...
class Lights
{
  public:
    ...
    void instantOn(int pixelID)
    {

      // Set the pixel to full brightness with the color white
      stripLights->setPixelColor(pixelID, stripLights->Color(rVal, gVal, bVal, wVal));
      // Update the strip to actually turn on the LEDs
      stripLights->show();

    }

    void instantOff(int pixelID)
    {

      // Turn off the pixel
      stripLights->setPixelColor(pixelID, stripLights->Color(0, 0, 0, 0));
      // Update the strip to turn off the LED
      stripLights->show();
